...
```{r}
### option two where Spatialunit is full, and spatial habitat is "All"
# if (input$woodytbl_spatialhabitat == "All") {
#   hotto <- df_treeplot %>%
#     dplyr::filter(Event_ID %in% df_wdevents) %>%
#   dplyr::left_join(all_events %>% dplyr::select(!!as.name(input$woodytbl_spatialunit), Plot_Code, Start_Date, Event_ID)) %>%
#   dplyr::mutate(Column = "Raw")
# 
#   sdstats <- hotto %>%
#   # dplyr::group_by(!!as.name(input$woodytbl_spatialunit)) %>%  #or spaitalunit if park/subunit
#   dplyr::summarise(across(where(is.numeric), sd, na.rm = TRUE), .groups = "drop") %>%
#   dplyr::mutate(Column = "Y_SD", 
#                 Plot_Code = "standard deviation")
# 
# avgstats <- hotto %>%
#   # dplyr::group_by(!!as.name(input$woodytbl_spatialunit)) %>% #or spaitalunit if park/subunit
#   dplyr::summarise(across(where(is.numeric), mean, na.rm = TRUE), .groups = "drop") %>%
#   dplyr::mutate(Column = "X_Average", 
#                 Plot_Code = "average")
# 
# thotto <- hotto %>%
#   dplyr::arrange(Plot_Code) %>%
#   rbind.fill(avgstats) %>%
#   rbind.fill(sdstats) %>%
#   dplyr::select(!!as.name(input$woodytbl_spatialunit), everything()) 
# 
# } else {
#   
#   hotto <- df_treeplot %>%
#   dplyr::filter(Event_ID %in% df_wdevents) %>%
#   dplyr::left_join(all_events %>% dplyr::select(!!as.name(input$woodytbl_spatialhabitat), Plot_Code, Start_Date, Event_ID)) %>%
#   dplyr::arrange(!!as.name(input$woodytbl_spatialhabitat), desc(Plot_Code)) %>% # remove if just park/subunit
#   dplyr::mutate(Column = "Raw")
# 
# sdstats <- hotto %>%
#   dplyr::group_by(!!as.name(input$woodytbl_spatialhabitat)) %>%  #or spaitalunit if park/subunit
#   dplyr::summarise(across(where(is.numeric), sd, na.rm = TRUE), .groups = "drop") %>%
#   dplyr::mutate(Column = "Y_SD", 
#                 Plot_Code = "standard deviation")
# 
...
#   rbind.fill(avgstats) %>%
#   rbind.fill(sdstats) %>%
#   dplyr::select(!!as.name(input$woodytbl_spatialhabitat), everything()) %>%
#   dplyr::arrange(!!as.name(input$woodytbl_spatialhabitat)) #, desc(Plot_Code)
# }
# 
# 
...
rv$df_selectedtreeplot <- reactable::reactable(df_selectedtreeplot,
    ...
    if (df_selectedtreeplot$Plot_Code[index] == "average") {
      list(borderTop = "2px solid black")  # Add a line above the average row
    } else if (df_selectedtreeplot$Plot_Code[index] == "standard deviation") {
      list(borderBottom = "2px solid black")  # Add a line below the standard deviation row
    } else {

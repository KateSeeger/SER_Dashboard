...
```{r}
# trees <- read_csv(here::here("output", "trees.csv")) #NEWNEW cycle tree basics
# seedlingsapling <- read_csv(here::here("output", "SeedlingSapling.csv")) #NEWNEW cycle seedling sapling
# broadgroup <- read_csv(here::here("output", "Broadgroup.csv")) #used to be broadgroup
# # species_rich <- read_csv(here::here("Data_In","SpeciesDiversity_LongFormat_20230421.csv")) #waiting on new data from Tim
# taxa <- read_csv(here::here("output", "Taxa.csv"))
# location <- read_csv(here::here("output", "Location.csv")) #let's just call this LOCATION
# # genus_group <- read_csv(here::here("output", "Genus_grouping.csv"))
# events <- read_csv(here::here("output", "Events.csv"))
...
trees <- read_csv(here::here("outputCUPN2024", "CUPN_TreeBasics.csv")) #NEWNEW cycle tree basics
seedlingsapling <- read_csv(here::here("outputCUPN2024", "CUPN_SeedlingSapling.csv")) #NEWNEW cycle seedling sapling
broadgroup <- read_csv(here::here("outputCUPN2024", "CUPN_BroadGroup.csv")) #used to be broadgroup
# species_rich <- read_csv(here::here("Data_In","SpeciesDiversity_LongFormat_20230421.csv")) #waiting on new data from Tim
taxa <- read_csv(here::here("outputCUPN2024", "TaxaTable.csv"))
location <- read_csv(here::here("outputCUPN2024", "Location_Table2.csv")) #let's just call this LOCATION
genus_group <- read_csv(here::here("outputCUPN2024", "Genus_grouping.csv"))
events <- read_csv(here::here("outputCUPN2024", "CUPN_EventTable2.csv"))
...
# input$stack_userunit <- "Beech Creek"
# input$stack_season <- "Sample_Year"
# input$stack_year <- "2022"
...
observeEvent(eventExpr = input$button_UpdatePlots, {
 ...
 shiny::req(!is.null(input$stack_spatialunit), !is.null(input$stack_userunit), !is.null(input$stack_spatialhabitat), !is.null(df_treefull), !is.null(all_events), !is.null(input$stack_year), !is.null(input$stack_spgroup), !is.null(input$stack_season), !is.null(input$stack_userhabitat), !is.null(input$imgtitle))
...
t <- list(
 ...
 # if (x == 1 ) {
  ...
  layout(
    title = list(text = input$imgtitle, font = t, y = 0.9, x = 0.4, xanchor = 'center', yanchor =  'bottom'),
  margin = list(
  l = 50,#50, 
  r = 50,#50,
  b = 50,#100,
...
renderUI({ #needed bc reactive
  ...
  # input<- list() 
  # input$stack_spatialunit <- "Subunit_Code"
  spatialuserchoice <- all_events %>%
    distinct(!!as.name(input$stack_spatialunit))
  
  selectInput(
    "stack_userunit",
    label = strong(paste0("Select Individual Unit of ", input$stack_spatialunit, ":" )),
    choices = sort(spatialuserchoice[[input$stack_spatialunit]]), 
    selected = sort(spatialuserchoice[[input$stack_spatialunit]])[1]
...
renderUI({
  ...
  selectInput(
    ...
    selected = sort(communitychoice)[1]
  )
  }
...
### newnewnew
renderUI({
  shiny::req(!is.null(input$stack_spatialhabitat), !is.null(input$stack_userhabitat), !is.null(input$stack_userunit), !is.null(input$stack_season), !is.null(input$stack_year))
  
   if (input$stack_spatialhabitat == "All" & input$stack_userhabitat == "All") {
  plottitle <- input$stack_userunit
   
   } else {
  plottitle <- paste(input$stack_userunit, input$stack_userhabitat)
...
textInput(
  ...
  value = paste(plottitle, input$stack_season, input$stack_year)
...
downloadLink("button_exportstackbar", label = "Download .png") 

output$stackplotly_tree <- renderPlotly({
  shiny::req(!is.null(rv$plotly_tree))
  rv$plotly_tree})

plotlyOutput("stackplotly_tree", height = "70%")

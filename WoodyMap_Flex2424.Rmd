...
canopycover <- read_csv(here::here("outputCUPN2024", "CUPN_CanopyCover.csv"))
...
```{r  start of woodymap_Flex2424 unique code}
...
spdiv <- df_spdiv %>%
  left_join(df_genus, by = "Plant_Code", relationship = "many-to-many") %>%
...
# rv$df_treedivtbl <- df_treedivtbl
# 
...
rv$df_spdivbar <- chartdata_long %>%
  ...
  geom_bar(width = 0.45, 
           ...
           stat = "identity") +
  theme_clean() +
  labs(title = paste0(click_marker(), ", ",input$map_year), x = "", y = "") +
...
```{r selecinput working code 11/30/24}
...
# div(style = "margin-top: 10px;",
#   renderUI({
#     shiny::req(input$map_spgroup) # Ensure input$map_spgroup is available
#     
#     if (!exists("genus_grouping") || is.null(genus_grouping)) {
#       # If `genus_grouping` does not exist or is null
#       selectInput(
#         "map_usergroup",
#         label = strong("Select a species grouping:"),
#         choices = "Genus",                    
...
#     } else {
#       
#       # If `genus_grouping` is available
#       gg <- genus_grouping %>% dplyr::select(-Plant_Code)
#       genuscols <- colnames(gg)  # Get the column names
# 
#       
#       # Exclude "Genus" if map_spgroup is set to "Plant_Code"
#       if (input$map_spgroup == "Plant_Code") {
#         genuscols <- setdiff(genuscols, "Genus")
#       } 
# 
#       selectInput(
#         "map_usergroup",
#         label = strong("Select a species grouping:"),
#         choices = sort(genuscols),  # Provide sorted column names
#         selected = if ("Genus" %in% genuscols) "Genus" else ""
#       )
#  }
#   })
# )

...
renderUI({
  shiny::req(!is.null(trees), !is.null(click_marker), !is.null(rv$map_sequence))

  filtertrees <- trees %>%
        dplyr::filter(
      Event_ID %in% rv$map_sequence,
      Plot_Code == "BIRT002" #click_marker()
      )
    ...
    renderUI({
       if (!exists("genus_grouping") || is.null(genus_grouping)) {
         ...
         selectInput(
           "map_spgroup",
           label = strong("Select a taxonomic grouping: "),
           choices = c("Genus", "Plant_Code"), # Offer both options
           selected = ""
         )
       }
    })

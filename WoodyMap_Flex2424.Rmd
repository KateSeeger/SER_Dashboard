    ...
    renderUI({
       if (!exists("genus_grouping") || is.null(genus_grouping)) {
         ...
         selectInput(
           ...
           choices = c("Genus"), # Offer both options
           selected = "Genus"
         )
         ...
         selectInput(
           "map_spgroup",
           label = strong("Select a taxonomic grouping: "),
           choices = c("Genus", "Species" = "Plant_Code"), # Offer both options   #### we need this to have as Species option can we change this somehow link??
           selected = ""
         )
       }
    })
...
# UI for map_usergroup input with conditional logic
div(style = "margin-top: 10px;",
  renderUI({
...
downloadLink("button_exportwdmapraw", label = "Download Data Summary") 
...
output$button_exportwdmapraw <- downloadHandler(
    filename = function() {
      paste0("WoodyRawData-", Sys.Date(), ".csv", sep="")
    },
